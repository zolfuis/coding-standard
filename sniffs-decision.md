...
| `Arrays.ArrayAccess` | ✅ Added | Prevents brittle `getItems()[0]` patterns; forces explicit variable assignment for clarity |
| `Arrays.DisallowImplicitArrayCreation` | ✅ Added | Prevents accidental implicit `$arr[] = $x` when the array was never initialized |
| `Arrays.DisallowPartiallyKeyed` | ✅ Added | Prevents ambiguous mixed keyed/unkeyed arrays; all-keyed or all-unkeyed is always unambiguous |
| `Arrays.MultiLineArrayEndBracketPlacement` | ✅ Added | PSR-12 does not fully cover multi-line array closing bracket placement; closing `]` must be on its own line |
